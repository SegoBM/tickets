...
class _TicketsLevantados extends State<TicketsRecibidos> {
  ...
  final key = GlobalKey(); FocusNode focusNode = FocusNode();
  late Timer _timer;
  TextEditingController searchController = TextEditingController();
  ScrollController scrollController = ScrollController(), scrollControllerButtons = ScrollController();
  ...
  List<String> attachedFiles = ['archivo1.pdf', 'archivo2.docx', 'imagen.png'],
      status = ['Abierto', 'En Progreso', 'Resuelto'], importancia = ['Importante', 'Urgente', 'No urgente', 'Pregunta'];
  List<Color> statusColors = [Colors.green, Colors.orange, Colors.blue];
  List<TicketsModels> listTicketsTemp = [], listTicketsTempReport = [], listTickets = [];
  ...
  late DateTime before; late DateTime after;
  String? idPuesto; String? idUsuario; String? department;
  @override
  void initState() {
    super.initState();
  ...
  List<Widget> _filtros() {
    return [
      SizedBox(width: 250, height: 55,
        child: MyTextfieldIcon(labelText: Texts.searchTicket, focusNode: focusNode,
          colorLineBase: Colors.black54, textController: searchController,
          textColor: Colors.black54, cursorColor: Colors.black54,
          ...
          backgroundColor: ColorPalette.ticketsSelectedColor, formatting: false,
      ...
      Container(
        padding: const EdgeInsets.symmetric(horizontal: 5, vertical: 8), width: 250,
        decoration: BoxDecoration(color: ColorPalette.ticketsSelectedColor,
            borderRadius: BorderRadius.circular(10)),
        child: CustomDropdownButton(context: context, items: items, selectedItems: selectedItems1,
      ...
      Container(
        ...
        child: CustomDropdownButton(
            context: context, items: importancia, selectedItems: selectedItems2,
            setState: setState, text: Texts.ticketSelectImportance,
            color: Colors.black54, textColor: Colors.black54,
      ...
      FloatingActionButton(
        onPressed: () {
          ...
          showDialog<DateTimeRange>(context: context, builder: size.width > 500
                ? (context) => AlertDialog(
                      backgroundColor: Colors.transparent, surfaceTintColor: Colors.transparent,
                      content: SizedBox(width: 338, height: 530,
      ...
      FloatingActionButton(
        ...
        tooltip: Texts.ticketSelectDateRangeReport, backgroundColor: ColorPalette.ticketsColor,
  ...
  Widget _customButtonShowConversation(String idTickets, {double width = 108, double height = 50}) {
    return CustomButton(text: size.width > 1050 ? Texts.conversacion : "",
        onPressed: () async {showConversation(idTickets);},
        icon: IconLibrary.iconConversation, width: size.width < 1050 ? 30 : width,
        height: height, color: ColorPalette.ticketsColor.withOpacity(0.9), colorText: Colors.white);
  }

  Widget customButtonShowConversationMobile(String idTickets, {double width = 108, double height = 50, double widthIcon = 30}) {
    return IconButton(
        onPressed: () async {showConversationMobile(idTickets);},
        icon: const Icon(IconLibrary.iconConversation, color: Colors.white,),
        style: ButtonStyle(
          backgroundColor: MaterialStateProperty.all<Color>(ColorPalette.ticketsColor),
  ...
  Widget body() {
    return Scaffold(backgroundColor: ColorPalette.ticketsTextSelectedColor,
        appBar: MyCustomAppBarDesktop(title: "Recibidos", context: context,
            ...
            child: Column(children: [
              ...
              SizedBox(height: size.height - 120,
                  child: Container(
                    ...
                    padding: const EdgeInsets.all(5), child: futureList(),
                  ))
            ]),
          ),
            ...
            child: Column(
              children: [
                FadingEdgeScrollView.fromSingleChildScrollView(
                    child: SingleChildScrollView(controller: scrollControllerButtons,
                ...
                SizedBox(height: size.height - 155,
                    child: Container(decoration: BoxDecoration(borderRadius: BorderRadius.circular(15.0),
                        color: ColorPalette.ticketsColor,
                      ), padding: const EdgeInsets.all(3), child: futureList(),
                    ))
              ],
            ),
          ),
        ...
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          if (listTickets.isNotEmpty) {
            ...
            if (_isLoading) {
              ...
              return RefreshIndicator(onRefresh: () async {await _getDatosReload();},
                color: ColorPalette.ticketsColor2, backgroundColor: ColorPalette.ticketsUnselectedColor,child: SingleChildScrollView(child:
              Center(child: NoDataWidgetTickets(text: Texts.ticketNotFind,))),);
            }
          }
        }
      },
  ...
  Widget futureListTickets(){
    return RefreshIndicator(color: ColorPalette.ticketsColor2,backgroundColor:ColorPalette.ticketsUnselectedColor,child: Scrollbar(thumbVisibility: true, controller: scrollController,
      child: FadingEdgeScrollView.fromScrollView(
        child: ListView.builder(itemCount: listTicketsTemp.length, shrinkWrap: true,
          controller: scrollController, physics: const AlwaysScrollableScrollPhysics(),
          itemBuilder: (context, index) {
            return size.width > 500
                ? card(listTicketsTemp[index]) : cardMobile(listTicketsTemp[index]);
          },
        ),
      ),
    ), onRefresh: () async {await _getDatosReload();}
    );
  }
  ...
  Widget cardEsqueleto(double width) {
    return SizedBox(width: width, height: max(152 * (size.width / 1200), 180.0),
      child: Container(margin: const EdgeInsets.all(3),
        decoration: BoxDecoration(color: ColorPalette.ticketsColor7, borderRadius: BorderRadius.circular(10.0)),
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Container(alignment: Alignment.center,
                      decoration: BoxDecoration(color: ColorPalette.ticketsColor,
                      ...
                      child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween,
                        children: [
                          Expanded(
                              child: ListView(padding: const EdgeInsets.only(top: 5.0),
                                scrollDirection: Axis.horizontal,
                            children: [
                              Text(tickets.Titulo, style: const TextStyle(color: Colors.white,
                    ...
                    Text("Usuario: ${tickets.UsuarioNombre}",overflow: TextOverflow.ellipsis,
                      style: TextStyle(color: Colors.white.withOpacity(0.8),
                          fontSize: 13 * (size.width / 1400), fontWeight: FontWeight.bold),
                    ),
                    const SizedBox(height: 5,),
                    Text("Departamento: ${tickets.NombreDepartamentoUsuarioLevantado}",
                      style: TextStyle(color: Colors.white.withOpacity(0.8),
                          fontSize: 13 * (size.width / 1400), fontWeight: FontWeight.bold),
                    ),
                  ],
  ...
  Widget cardMobile(TicketsModels tickets) {
    return GestureDetector(onTap: () async {HapticFeedback.vibrate();ticketResume(tickets);},
      child: Card(shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10.0),),
      color: ColorPalette.ticketsColor7, elevation: 0,
      child: Padding(padding: const EdgeInsets.all(5.0),
        child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween,
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                    ...
                    Flexible(
                      child: Wrap(spacing: 2, alignment: WrapAlignment.end, runSpacing: 5,
                        children: [
                          buttons(tickets)[3],
                          buttons(tickets)[0],
                          buttons(tickets)[1],
                  ...
                  if (tickets.FechaFinalizacion != null) ...[
                    Text("Fecha de finalización  : ${tickets.FechaFinalizacion?.split("T")[0] ?? ""} "
                  ...
                  Row(mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      Column(crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Tooltip(message: "Usuario: ${tickets.UsuarioNombre}",child:
                          SizedBox(width: size.width-85,child: Text("Usuario: ${tickets.UsuarioNombre}",
                            style: TextStyle(color: Colors.white.withOpacity(0.8), overflow: TextOverflow.ellipsis,
                                fontSize: 12 * (size.width / 350), fontWeight: FontWeight.bold),
                          ))),
                          const SizedBox(height: 5,),
                      ...
                      Column(mainAxisAlignment: MainAxisAlignment.center,
                        children: [
                          Center(child: customButtonShowConversationMobile(tickets.IDTickets!, height: 25)),
                        ],
                      ),
                    ],
                  )
  ...
  List<Widget> buttons(TicketsModels tickets, {double width = 90}) {
    ...
    return [
      tickets.Estatus == "Cerrado" ? buttonCerrado(tickets, width: width)
          : buttonOpciones(tickets, width: width),
      Container(alignment: Alignment.center,
          decoration: BoxDecoration(borderRadius: BorderRadius.circular(5),
            color: tickets.Prioridad == "Urgente" ? Colors.red
                : tickets.Prioridad == "Importante" ? Colors.orange
                    : tickets.Prioridad == "No urgente" ? Colors.green
                        : tickets.Prioridad == "Pregunta" ? Colors.blue
                            : Colors.black,
          ),
          height: 25, width: width,
          child: Text(" ${tickets.Prioridad}", style: const TextStyle(color: Colors.white, fontSize: 10))
      ),
      tickets.Estatus == "Cerrado" ? Container() : Container(alignment: Alignment.center,
              child: Icon(Icons.access_time_filled_outlined, color: clock, size: 20,),
            ),
      if (isCreatedToday && tickets.Estatus=="Abierto")...[
        Container(alignment: Alignment.center, decoration: const BoxDecoration(color: ColorPalette.ticketsColor6,
              borderRadius: BorderRadius.all(Radius.circular(5))), margin: const EdgeInsets.only(right: 1),
          height: 25, width: 80,
          child: Stack(clipBehavior: Clip.none, // Permite que los hijos se dibujen fuera del box del Stack
            children: [
              const Center(child: Text("Nuevo", style: TextStyle(color: Colors.white),),),
                Positioned(right: -4, top: -4,
                  child: Container(decoration: const BoxDecoration(color: Colors.red, shape: BoxShape.circle,),
                    width: 15, height: 15,
                    child: Center(child: Transform.translate(offset: const Offset(0, -1), // Mueve el texto 2 píxeles hacia arriba
    ...
    return FloatingActionButton(key: key,
      onPressed: () async {
        ...
        final RelativeRect position = RelativeRect.fromRect(
          Rect.fromPoints(button.localToGlobal(button.size.bottomCenter(Offset.zero), ancestor: overlay),
            button.localToGlobal(button.size.bottomCenter(Offset.zero), ancestor: overlay),
          ), Offset.zero & overlay.size,
        );
        var result = await showMenu(context: context, position: position,
          elevation: 8.0, constraints: const BoxConstraints(maxWidth: 274.0),
          surfaceTintColor: Colors.transparent, color: ColorPalette.ticketsColor4,
  ...
  Widget buttonCerrado(TicketsModels tickets, {double width = 90}) {
    return Container(alignment: Alignment.center,
        ...
        child: Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly,
          children: [
            Text("Cerrado", style: TextStyle(color: Colors.white, fontSize: size.width > 500 ? 12 : 10)),
            InkWell(onTap: () async {star(tickets);},
  ...
  Widget buttonOpciones(TicketsModels tickets, {double width = 90}) {
    return Container(alignment: Alignment.center, height: 25, width: width,
      decoration: BoxDecoration(color: statusColors[status.indexOf(tickets.Estatus)],
          borderRadius: const BorderRadius.all(Radius.circular(5))),
      child: DropdownButton<String>(iconSize: 20, isExpanded: !Platform.isWindows,
        alignment: Alignment.center, dropdownColor: ColorPalette.ticketsColor, value: tickets.Estatus,
        style: TextStyle(color: Colors.white, fontSize: size.width > 500 ? 12 : 10, fontWeight: FontWeight.bold),
        underline: Container(),
        onChanged: (String? newValue) async {
          bool result = await comprobarSave();
          if (result) {
            await changeStatus(id: tickets.IDTickets!, estatus: newValue!, oldStatus: tickets.Estatus,);
    ...
    showDialog(context: context,
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          title: Text(Texts.ticketQualification, style: TextStyle(color: Colors.white),),
          content: Container(height: 350, width: 500,
            padding: EdgeInsets.all(10),
            decoration: BoxDecoration(borderRadius: BorderRadius.circular(10),),
            child: Column(crossAxisAlignment: CrossAxisAlignment.center,
              children: [
                Container(width: 450,
                  decoration: BoxDecoration(color: ColorPalette.ticketsColor2,
                    borderRadius: BorderRadius.circular(10),
                  ), padding: EdgeInsets.all(10),
                  child: Row(
                    children: [
                ...
                const SizedBox(height: 5),
                Container(decoration: BoxDecoration(color: ColorPalette.ticketsColor2,
                  borderRadius: BorderRadius.circular(10),),
                  padding: EdgeInsets.all(10), width: 450,
                  child: Row(
                    children: [
  ...
  Widget _desktopBody(BuildContext context) {
    return Scaffold(backgroundColor: ColorPalette.ticketsTextSelectedColor, body: body());
  }

  Widget _mobileBody(BuildContext context) {
    return Scaffold(backgroundColor: ColorPalette.ticketsTextSelectedColor, body: bodyMobile());
  }
  ...
  Future<void> aplicarFiltroFechaReporte() async {
    ...
    if (startDateReport != null && endDateReport != null) {
      try {
        ...
        lisTemp = await ticketViewController.getTicketsRecibidosReportUser("${startDateReport}", "${endDateReport},", idUsuario, department);
        ...
        setState(() {});
        LoadingDialogTickets.showLoadingDialogTickets(context, Texts.loadingData);
        await generatedTicketsReport2(context, lisTempReport, startDateReport!, endDateReport!);
        LoadingDialogTickets.hideLoadingDialogTickets(context);
        LoadingDialogTickets.hideLoadingDialogTickets(context);
      } catch (e) {
  ...
  Future<void> changeStatus({required String id, required String estatus, required String oldStatus}) async {
    try {
      LoadingDialogTickets.showLoadingDialogTickets(context, Texts.loadingData);
      ...
      if (oldStatus == "Abierto") {
        if (estatus == "Resuelto" || estatus == "En Progreso") {
          ...
          //Recordar que voy a reasignar el ticket aquí
        }
      }
      ...
      if (save) {
        await Future.delayed(const Duration(milliseconds: 500), () async {
          ...
          await Future.delayed(const Duration(milliseconds: 200), () async {
            CustomAwesomeDialogTickets(title: Texts.updateSuccess, desc: '',
                    width: size.width < 500 ? size.width * .9 : null,
        ...
        CustomAwesomeDialogTickets(title: Texts.errorSavingData, desc: Texts.ticketErrorSave,
            ...
            btnCancelOnPress: () {LoadingDialogTickets.hideLoadingDialogTickets(context);}).showError(context);
      }
    } catch (e) {
  ...
  Future<bool> comprobarSave() {
    ...
    CustomAwesomeDialogTickets(title: Texts.askSaveConfirmStatus, desc: '',
        btnOkOnPress: () {completer.complete(true);}, width: size.width < 500 ? size.width * .9 : null,
        btnCancelOnPress: () {completer.complete(false);}).showQuestion(context);
        ...
        builder: (BuildContext context) {
          return AlertDialog(backgroundColor: ColorPalette.ticketsColor3,
            content: Container(
              ...
              child: ClipRRect(borderRadius: BorderRadius.circular(10), // Radio del borde
  ...
  Future<void> aplicarFiltro() async {
    ...
    if (searchController.text.isNotEmpty) {
      listTicketsTemp = listTicketsTemp.where((ticket) =>
              ticket.Titulo.toLowerCase().contains(searchController.text.toLowerCase()) ||
              "${ticket.UsuarioAsignadoID} ${ticket.NombreDepartamento} ${ticket.Estatus}".toLowerCase()
                  .contains(searchController.text.toLowerCase()) ||
              ticket.UsuarioNombre!.toLowerCase().contains(searchController.text.toLowerCase())).toList();
    }
    setState(() {});
  }
  ...
  Future<void> aplicarFiltro2() async {
    ...
    if (searchController.text.isNotEmpty) {
      listTicketsTemp = listTicketsTemp
          .where((ticket) =>
              ticket.Titulo.toLowerCase().contains(searchController.text.toLowerCase()) ||
              "${ticket.UsuarioAsignadoID} ${ticket.NombreDepartamento} ${ticket.Estatus}".toLowerCase().contains(searchController.text.toLowerCase()) ||
              ticket.UsuarioNombre!.toLowerCase().contains(searchController.text.toLowerCase()))
          .toList();
    }
    setState(() {});
  }
  ...
  Future<void> aplicarFiltroFecha() async {
    if (startDate != null && endDate != null) {
      try {
        //DateTime before = today.subtract(const Duration(days: 5));
        //dateInitial.text = "${before.year}-${(before.month).toString().padLeft(2, "0")}-${before.day.toString().padLeft(2, "0")} / ${today.year}-${today.month.toString().padLeft(2, "0")}-${today.day.toString().padLeft(2, "0")}";
        listTickets = await ticketViewController.getTicketsRecibidos("${startDate}", "${endDate},", idUsuario!, department!);
        listTicketsTemp = listTickets;
    ...
    try {
      ...
      await Navigator.of(widget.context).push(MaterialPageRoute(
          builder: (context) => TicketsConversationScreen(
                idTicket: idTickets, listCommentaries: listCommentary, activeMessages: true,
              )));
      listCommentary.clear();
    } catch (e) {
      LoadingDialogTickets.hideLoadingDialogTickets(context);
  ...
  Future<void> _getDatos() async {
    try {
      ...
      listTickets = await ticketViewController.getTicketsRecibidos("${before.year}-${before.month}-${before.day}",
          "${after.year}-${after.month}-${after.day},", idUsuario!, department!);
      if (listTickets.isEmpty) {
        isEmpty = true;
        _isLoading = false;
      }else{
        for(int i = 0; i < listTickets.length; i++){
          if(listTickets[i].Estatus == "Abierto" && isCreatedToday(listTickets[i].FechaCreacion!)){
            notification();
          }
        }
  ...
  Future<void> notification() async {
    try{
      if(Platform.isIOS || Platform.isAndroid){
        if (await Vibration.hasVibrator() != null && await Vibration.hasVibrator() != false) {
  ...
  Future<void> _getDatosReload() async {
    try {
      ...
      List<TicketsModels> list = await ticketViewController.getTicketsRecibidos("${before.year}-${before.month}-${before.day}",
          "${after.year}-${after.month}-${after.day},", idUsuario!, department!);
      ...
      if (list.isEmpty) {
        ...
        if(listTickets.length != list.length){
          for(int i = 0; i < list.length; i++){
            if(list[i].Estatus == "Abierto" && isCreatedToday(listTickets[i].FechaCreacion!)){
              notification();
              break;
            }
          }
  ...
  bool isCreatedToday(String fechaCreacion) {
    ...
    DateTime now = DateTime.now();
    bool isCreated = (ticketDate.year == now.year && ticketDate.month == now.month && ticketDate.day == now.day);
    return isCreated;
  }
}
