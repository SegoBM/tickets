...
class TicketsRecibidosTest extends StatefulWidget {
  static String id = 'TicketsRecibidos';
  BuildContext context;
  TicketsRecibidosTest({super.key, required this.context});

  @override
  State<TicketsRecibidosTest> createState() => _TicketsLevantados();
}

class _TicketsLevantados extends State<TicketsRecibidosTest> {
  late Size size; TextEditingController _searchController = TextEditingController();
  ScrollController _scrollController = ScrollController();
  List<String> items = ["Abierto", "En Progreso", "Resuelto", "Cerrado"], selectedItems1 = [], selectedItems2 = [];
  List<String> attachedFiles = ['archivo1.pdf', 'archivo2.docx', 'imagen.png'], status = ['Abierto', 'En Progreso', 'Resuelto'];
  ...
  Color currentColor = Colors.green; DateTime today = DateTime.now();
  ...
  bool isEmpty = false, _isLoading = true;int itemCount = 0;
  ...
  List<Widget> _filtros() {
    return [
      SizedBox(width: 250, height: 55,
        child: MyTextfieldIcon(labelText: "Buscar ticket",
          colorLineBase: Colors.black54, textController: _searchController,
      ...
      Container(padding: const EdgeInsets.symmetric(horizontal: 5, vertical: 8), width: 250,
        ...
        child: CustomDropdownButton(context: context, items: Importancia,
            selectedItems: selectedItems2, setState: setState, color: Colors.black54,
        ...
        onPressed: () {
          ...
          showDialog<DateTimeRange>(context: context,
            builder: (context) => AlertDialog(
              ...
              content: SizedBox(width: 338, height: 800,
  ...
  Widget body() {
    return Scaffold(backgroundColor: ColorPalette.ticketsTextSelectedColor,
        appBar: MyCustomAppBarDesktop(title: "Recibidos", context: context, textColor: Colors.white,
          backButton: true, defaultButtons: false, color: ColorPalette.ticketsColor,
          backButtonWidget: TextButton.icon(
            ...
            style: TextButton.styleFrom(
              foregroundColor: Colors.white, backgroundColor: Colors.transparent, // Color de fondo
            ),
              ...
              children: [
                ...
                SizedBox(
                    ...
                    child: Container(
                      ...
                      padding: const EdgeInsets.all(7), child: futureList(),
                    ))
              ],
            ),
          ),
                  ...
                  children: [
                    Container(
                      ...
                      child: Row(mainAxisAlignment: MainAxisAlignment.spaceAround,
                        children: [
                          Text(tickets.Titulo, style: TextStyle(color: Colors.white,
  ...
  Future<void> aplicarFiltroFechaReporte() async {
    if (startDateReport != null && endDateReport != null) {
      try {
        ...
        lisTemp = await ticketViewController.getTicketsRecibidosReportAdmin(
            "${startDateReport}", "${endDateReport},", idUsuario, departament);

        //listSatisfaction = await satisfactionController.
        lisTempReport = lisTemp;
        print(lisTempReport.length);
        if (lisTempReport.isEmpty) {
          isEmpty = true;
        }
        setState(() {});
        generatedTicketsReport2(context,lisTempReport,startDateReport!,endDateReport!);
      } catch (e) {
    ...
    if (startDate != null && endDate != null) {
      try {
        ...
        listTickets = await ticketViewController.getTicketsRecibidos(
            "${startDate}", "${endDate},", idUsuario, departament);
        listTicketsTemp = listTickets;
