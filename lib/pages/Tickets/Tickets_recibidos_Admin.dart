...
class _TicketsLevantados extends State<TicketsRecibidosAdmin> {
  late Size size; FocusNode focusNode = FocusNode();
  ...
  List<String> selectedItems1 = [], selectedItems2 = [], attachedFiles = ['archivo1.pdf',
    'archivo2.docx', 'imagen.png'], status = ['Abierto', 'En Progreso', 'Resuelto'],
    ...
    return [
      SizedBox(width: 250, height: 55,
        child: MyTextfieldIcon(labelText: Texts.searchTicket, colorLineBase: Colors.black54,
          ...
          suffixIcon: const Icon(IconLibrary.iconSearch, color: Colors.black), focusNode: focusNode,
      ...
      Container(padding: const EdgeInsets.symmetric(horizontal: 5, vertical: 8), width: 250,
        ...
        child: CustomDropdownButton(context: context, items: status,
            selectedItems: selectedItems1, textColor: Colors.black54,
      ...
      FloatingActionButton(
        onPressed: () {
          CustomAwesomeDialogTickets(title: "¿Estas seguro que deseas generar un reporte de tickets?\n",
              desc: Texts.ticketRemember,
              btnOkOnPress: () async {
                showDialog<DateTimeRange>(context: context,
                  builder: size.width>500? (context) => AlertDialog(
                    backgroundColor: Colors.transparent, surfaceTintColor: Colors.transparent,
                    content: SizedBox(width: 338, height: 530, child: dialogCustomDateRangePickerFilterReport()),
                  ) : (context) => dialogCustomDateRangePickerFilterReport(),
  ...
  Widget _customButtonShowConversation(String idTickets) {
    return CustomButton(text: size.width > 1050 ? Texts.conversacion : "",
        ...
        icon: IconLibrary.iconConversation, width: size.width < 1050 ? 30 : 108,
        height: 50, color: ColorPalette.ticketsColor.withOpacity(0.9),
  ...
  Future<void> showConversationMobile(String idTickets) async {
    try {
      ...
      await Navigator.of(widget.context).push(MaterialPageRoute(
          builder: (context) => TicketsConversationScreen(idTicket: idTickets, listCommentaries: listCommentary,
            activeMessages: true,)));
  ...
  Widget body() {
    return Scaffold(
        ...
        appBar: MyCustomAppBarDesktop(title: "Recibidos Admin", context: context,
          textColor: Colors.white, backButton: false, color: ColorPalette.ticketsColor,ticketsFlag: false,
          defaultButtons: true,extracolor2: Colors.white,extracolor: ColorPalette.ticketsColor,
              ...
              children: [
                if (size.width > 1260) ...[
                  Row(mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [Row(children: [..._filtros(),],),],
                  )
                ] else ...[
                  Row(
                    children: [
  ...
  Widget bodyMobile() {
    return Scaffold(backgroundColor: ColorPalette.ticketsTextSelectedColor,
        body: Padding(padding: const EdgeInsets.symmetric(horizontal: 10.0, vertical: 5),
          child: SingleChildScrollView(
              ...
              children: [
                ...
                SizedBox(height: size.height - 150,
                    child: Container(
                      ...
                      padding: const EdgeInsets.all(7), child: futureList(mobile: true),
                    ))
              ],
            ),
          ),
    ...
    return Scaffold(backgroundColor: ColorPalette.ticketsTextSelectedColor,
        body: Padding(padding: const EdgeInsets.symmetric(horizontal: 5, vertical: 5),
          child: SingleChildScrollView(
            child: Column(
              children: [
                FadingEdgeScrollView.fromSingleChildScrollView(child:
                SingleChildScrollView(scrollDirection: Axis.horizontal, controller: scrollControllerButtons,child: Row(children: [
                  _filtros()[0],
                  const SizedBox(width: 5,),
                  _filtros()[6],
                  const SizedBox(width: 5,),
                ...
                SizedBox(height: size.height - 150,
                    child: Container(decoration: BoxDecoration(borderRadius: BorderRadius.circular(15.0),
                      color: ColorPalette.ticketsColor,), padding: const EdgeInsets.all(5), child: futureList(),
                    ))
              ],
            ),
          ),
          ...
          if (listTickets.isNotEmpty) {
            ...
            if (_isLoading) {
              ...
              return SingleChildScrollView(
                child: Center(child: NoDataWidgetTickets(text: "No se encontrarón Tickets",)),
              );
            }
  ...
  Widget cardMobile(TicketsModels tickets) {
    return GestureDetector(
      onTap: () async {ticketResume(tickets);},
      ...
      child: Container(margin: const EdgeInsets.all(5),
        ...
        child: Padding(padding: const EdgeInsets.symmetric(horizontal: 5, vertical: 5),
          child: Shimmer.fromColors(baseColor: ColorPalette.ticketsColor7,
            ...
            enabled: true, child: Container(margin: const EdgeInsets.all(3),
  ...
  Widget card(TicketsModels tickets) {
    return SizedBox(height: max(153 * (size.width / 1200), 185.0),
      child: Card(shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10.0),),
        color: ColorPalette.ticketsColor7, elevation: 0,
        child: Padding(padding: const EdgeInsets.all(5.0),
          child: Row(mainAxisAlignment: MainAxisAlignment.start,
            children: [
              ...
              Expanded(flex: 2,
                  child: SizedBox(height:  max(153 * (size.width / 1200), 185.0),child:
  ...
  Widget buttonCerrado(TicketsModels tickets, {double width = 90}) {
    return Container(alignment: Alignment.center, decoration: const BoxDecoration(color: Colors.black,
        borderRadius: BorderRadius.all(Radius.circular(5))), height: 25, width: width,
        child: Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly,
          children: [
            const Text("Cerrado", style: TextStyle(color: Colors.white, fontSize: 11,),),
  ...
  Widget starDialog(SatisfactionModel satisfaction, BuildContext context){
    return AlertDialog(shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(7.0)),
      ...
      title: const Text('Calificación del ticket', style: TextStyle(color: Colors.white),),
      content: Container(height: 400, width: 500, padding: const EdgeInsets.all(20),
        decoration: BoxDecoration( borderRadius: BorderRadius.circular(10),),
        child: Column(crossAxisAlignment: CrossAxisAlignment.center,
          children: [
            Container(width: 450, decoration: BoxDecoration(color: ColorPalette.ticketsColor2,
              borderRadius: BorderRadius.circular(10),), padding: const EdgeInsets.all(10),
              child:Row(children: [
            ...
            Container(decoration: BoxDecoration(color: ColorPalette.ticketsColor2,
              borderRadius: BorderRadius.circular(10),), padding: const EdgeInsets.all(10),width: 450,
              child: Row(children: [
  ...
  List<Widget> buttons(TicketsModels tickets, {double width = 90}) {
    ...
    return [
      tickets.Estatus == "Cerrado"
          ? buttonCerrado(tickets, width: width) : buttonOpciones(tickets, width: width),
      tickets.Estatus == "Cerrado" ? buttonCerrado(tickets) : buttonOpciones(tickets),
      Container(alignment: Alignment.center, decoration: BoxDecoration(
        ...
        color: tickets.Prioridad == "Urgente"
          ...
          child: Text(" ${tickets.Prioridad}", style: const TextStyle(color: Colors.white, fontSize: 12,),)),
      tickets.Estatus == "Cerrado" ? Container()
          : Container(alignment: Alignment.center, child:  Icon(Icons.access_time_filled_outlined, color: clock, size: 20,),
  ...
  List<Widget> buttons2(TicketsModels tickets, {double width = 90}) {
    ...
    return [
      ...
      tickets.Estatus == "Cerrado" ? Container() : Container(
        alignment: Alignment.center, child:  Icon(Icons.access_time_filled_outlined, color: clock, size: 20,),
      ),
    ];
  }
  Future<void> aplicarFiltroFechaReporte() async {
    if (startDateReport != null && endDateReport != null) {
      try {
        ...
        listTempTickets = await ticketViewController.getTicketsRecibidosReportAdmin("${startDateReport}", "${endDateReport},", "",department);
        listTicketsTempReport = listTempTickets;
        if (listTicketsTempReport.isEmpty) {
          isEmpty = true;
        }
        setState(() {});
        generatedTicketsReport2(context,listTicketsTempReport,startDateReport!,endDateReport!);
      } catch (e) {
      ...
      if (save) {
        await Future.delayed(const Duration(milliseconds: 500), () async {
          ...
          await Future.delayed(const Duration(milliseconds: 200), () async {
            CustomAwesomeDialogTickets(title: Texts.updateSuccess, desc: '', width: size.width > 500 ? null : size.width * 0.9,
        ...
        CustomAwesomeDialogTickets(title: Texts.errorSavingData, desc: Texts.ticketErrorSave,
            btnOkOnPress: () {LoadingDialogTickets.hideLoadingDialogTickets(context);}, width: size.width > 500 ? null : size.width * 0.9,
  ...
  Future<bool> comprobarSave() {
    ...
    CustomAwesomeDialogTickets(title: Texts.askSaveConfirmStatus, desc: '',
        btnOkOnPress: () {completer.complete(true);}, width: size.width > 500 ? null : size.width * 0.9,
  ...
  Future<void> aplicarFiltro() async {
    ...
    if (selectedItems1.isNotEmpty) {
      listTicketsTemp = listTicketsTemp.where((ticket) => selectedItems1.contains(ticket.Estatus))
          .toList();
    }
    if (selectedItems2.isNotEmpty) {
      listTicketsTemp = listTicketsTemp.where((ticket) => selectedItems2.contains(ticket.Prioridad))
          .toList();
    }
  ...
  Future<void> showConversation(String idTickets) async {
    try {
      ...
      await myShowDialog(TicketsConversationScreen(idTicket: idTickets, listCommentaries: listComentary,
  ...
  Future<void> _getDatos() async {
    try {
      ...
      dateInitial.text = "${before.year}-${(before.month).toString().padLeft(2, "0")}-${before.day.toString().padLeft(2, "0")} / ${today.year}-${today.month.toString().padLeft(2, "0")}-${today.day.toString().padLeft(2, "0")}";
      listTickets = await ticketViewController.getTicketsRecibidosAdmin(
          "${before.year}-${before.month}-${before.day}", "${after.year}-${after.month}-${after.day},",
          departament);
